...
public class ProfileCheatsheetController {
	...
	public String edit(@PathVariable Long id, Model model) {
		// Lazy tag exception အား ကျော်လွှားရန် နှိုးဆွထားသော service method အား
		// သုံးစွဲခြင်း
	...
	public String update(@RequestParam Long id, @RequestParam String title,
		...
		if (coverPhoto != null && !coverPhoto.isEmpty()) {
			try {
				String uploadDir = System.getProperty("catalina.home") + java.io.File.separator + "webapps"
						+ java.io.File.separator + "uploads" + java.io.File.separator;
				java.io.File dir = new java.io.File(uploadDir);
				if (!dir.exists())
					dir.mkdirs();

				String fileName = System.currentTimeMillis() + "_"
						+ coverPhoto.getOriginalFilename().replaceAll("\\s+", "");
